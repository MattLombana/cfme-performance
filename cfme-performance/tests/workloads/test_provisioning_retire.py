...
from utils.providers import retire_provisioned_vm
from utils.providers import retire_provisioned_vms
from utils.providers import provision_vm
from utils.providers import get_template_guids
from utils.smem_memory_monitor import SmemMemoryMonitor
from utils.ssh import SSHClient
from utils.workloads import get_provisioning_retire_scenarios
from itertools import cycle
import time
import pytest


@pytest.mark.parametrize('scenario', get_provisioning_retire_scenarios())
def test_provisioning(request, scenario):
    ...
    scenario_data = {'appliance_ip': cfme_performance['appliance']['ip_address'],
        ...
        'test_dir': 'workload-provisioning-retire',
        'test_name': 'Provisioning-Retire',
    ...
    def cleanup_workload(scenario, from_ts, vms_to_cleanup):
        ...
        retire_provisioned_vms(vms_to_cleanup)
        monitor_thread.join()
        timediff = time.time() - starttime
        logger.info('Finished cleaning up monitoring thread in {}'.format(timediff))
        logger.info('{} VMs were left over, and {} VMs were retired in the finalizer.'
    ...
    while ((time.time() - starttime) < total_time):
        ...
        if provisioned_vms > cleanup_size * len(scenario['providers']):
            ...
            if retire_provisioned_vm(provision_order[0]):
                provision_order.pop(0)
                provisioned_vms -= 1
                total_retired_vms += 1
            retire_time = round(time.time() - start_remove_time, 2)
            logger.debug('Time to initate retire: {}'.format(retire_time))
            logger.info('{} VMs retired so far'.format(total_retired_vms))

        end_iteration_time = time.time()
        iteration_time = round(end_iteration_time - start_iteration_time, 2)
        elapsed_time = end_iteration_time - starttime
        logger.debug('Time to initiate provisioning and retire: {}'.format(iteration_time))
    ...
    logger.info('Provisioned {} VMs and retired {} VMs during the scenario.'
        .format(total_provisioned_vms, total_retired_vms))
