...
from utils.workloads import get_refresh_vms_bulk_scenarios
...
@pytest.mark.parametrize('scenario', get_refresh_vms_bulk_scenarios())
def test_refresh_vms_bulk(request, scenario):
    ...
    monitor_thread = SmemMemoryMonitor(SSHClient(), 'workload-refresh-vm-bulk', scenario['name'],
        'refresh-vm-bulk', get_server_roles_workload_refresh_vms(separator=','),
    ...
    logger.info('Queued {} provider refreshes during the scenario.'.format(total_refreshed_vms))
